...
% Save Figure
saveas(fig,'./Figures/price_sensitivity_sim6.png');
...
% Save Figure
saveas(fig,'./Figures/consumption_preference_sim6.png');
...
for run = 1:run_time
    ...
    graph = generate_neighborhood(N,6);
    ...
    Bh = [N/2,N/2,N/2,N/2,N/2,N/2]; Bh = Bh * (omega+4) / 4;
    profit_rate = 0.8; %Fixed profit rate
    beta(:) = profit_rate * kappa * ones(Hours,1);
    ...
    for h = 1:Hours
       ...
       % Selfish - Private
       tau = beta(h)*(Bh(h)+1)/N; lambda = beta(h)/N; mu = beta(h)/N; %main parameters
       ...
       price(h,:) = beta(h) * (Bh(h)*consumption_S(run,1,:,h)+total_consumption_S(1,run,h) + omega)/N;
       price_container(run,1,h,:) = price(h,:);

       cost(h) = kappa * total_consumption_S(1,run,h) ...
           * total_consumption_S(1,run,h)/N;
       revenue(h) = sum(price(h,:).* reshape(consumption_S(run,1,:,h),[1,N]),'all');
       realized_profit_rate(run,1,h) = revenue(h)/ cost(h);
       cost_container_S(run,1,h,1) = cost(h);

       revenue_container_S(run,1,h) = revenue(h);
       for i = 1:N
           u_i = -consumption_S(run,1,i,h)*price(h,i)+g(k,i)*consumption_S(run,1,i,h)...
               -e(i)*consumption_S(run,1,i,h)*consumption_S(run,1,i,h);
       ...
       price(h,:) = beta(h) * (Bh(h)*consumption_S(run,2,:,h)+total_consumption_S(2,run,h) + omega)/N;
       price_container(run,2,h,:) = price(h,:);

       cost(h) = kappa * total_consumption_S(2,run,h) ...
           * total_consumption_S(2,run,h)/N;
       revenue(h) = sum(price(h,:).* reshape(consumption_S(run,2,:,h),[1,N]),'all');
       realized_profit_rate(run,2,h) = revenue(h)/ cost(h);
       cost_container_S(run,2,h,1) = cost(h);

       revenue_container_S(run,2,h) = revenue(h);
       for i = 1:N
           u_i = -consumption_S(run,2,i,h)*price(h,i)+g(k,i)*consumption_S(run,2,i,h)...
               -e(i)*consumption_S(run,2,i,h)*consumption_S(run,2,i,h);
           utility_S(run,2,i,h) = u_i;
           welfare_S(run,2,h) = welfare_S(run,2,h) + u_i;
       end
       
       % Altruistic - Private
       tau = beta(h)*(Bh(h)+1)/N; lambda = 2*beta(h)/N; mu = beta(h)/N; %main parameters
       ...
       price(h,:) = beta(h) * (Bh(h)*consumption_U(run,1,:,h)+total_consumption_U(1,run,h) + omega)/N;
       price_container(run,4,h,:) = price(h,:);

       cost(h) = kappa * total_consumption_U(1,run,h) ...
           * total_consumption_U(1,run,h)/N;
       revenue(h) = sum(price(h,:).* reshape(consumption_U(run,1,:,h),[1,N]),'all');
       realized_profit_rate(run,4,h) = revenue(h)/ cost(h);
       cost_container_U(run,1,h,1) = cost(h);

       revenue_container_U(run,1,h) = revenue(h);
       for i = 1:N
          u_i = -consumption_U(run,1,i,h)*price(h,i)+g(k,i)*consumption_U(run,1,i,h)...
               -e(i)*consumption_U(run,1,i,h)*consumption_U(run,1,i,h);
       ...
       price(h,:) = beta(h) * (Bh(h)*consumption_U(run,2,:,h)+total_consumption_U(2,run,h) + omega)/N;
       price_container(run,5,h,:) = price(h,:);

       cost(h) = kappa * total_consumption_U(2,run,h) ...
           * total_consumption_U(2,run,h)/N;
       revenue(h) = sum(price(h,:).* reshape(consumption_U(run,2,:,h),[1,N]),'all');
       realized_profit_rate(run,5,h) = revenue(h)/ cost(h);
       cost_container_U(run,2,h,1) = cost(h);

       revenue_container_U(run,2,h) = revenue(h);
       for i = 1:N
           u_i = -consumption_U(run,2,i,h)*price(h,i)+g(k,i)*consumption_U(run,2,i,h)...
               -e(i)*consumption_U(run,2,i,h)*consumption_U(run,2,i,h);
       ...
       price(h,:) = beta(h) * (Bh(h)*consumption_W(run,1,:,h)+total_consumption_W(1,run,h) + omega)/N;
       price_container(run,7,h,:) = price(h,:);

       cost(h) = kappa * total_consumption_W(1,run,h) ...
           * total_consumption_W(1,run,h)/N;
       revenue(h) = sum(price(h,:).* reshape(consumption_W(run,1,:,h),[1,N]),'all');
       realized_profit_rate(run,7,h) = revenue(h)/ cost(h);
       cost_container_W(run,1,h,1) = cost(h);

       revenue_container_W(run,1,h) = revenue(h);
       for i = 1:N
           u_i = -consumption_W(run,1,i,h)*price(h,i)+g(k,i)*consumption_W(run,1,i,h)...
               -e(i)*consumption_W(run,1,i,h)*consumption_W(run,1,i,h);
       ...
       price(h,:) = beta(h) * (Bh(h)*consumption_W(run,2,:,h)+total_consumption_W(2,run,h) + omega)/N;
       price_container(run,8,h,:) = price(h,:);

       cost(h) = kappa * total_consumption_W(2,run,h) ...
           * total_consumption_W(2,run,h)/N;
       revenue(h) = sum(price(h,:).* reshape(consumption_W(run,2,:,h),[1,N]),'all');
       realized_profit_rate(run,8,h) = revenue(h)/ cost(h);
       cost_container_W(run,2,h,1) = cost(h);

       revenue_container_W(run,2,h) = revenue(h);
       for i = 1:N
           u_i = -consumption_W(run,2,i,h)*price(h,i)+g(k,i)*consumption_W(run,2,i,h)...
               -e(i)*consumption_W(run,2,i,h)*consumption_W(run,2,i,h);
           utility_W(run,2,i,h) = u_i;
           welfare_W(run,2,h) = welfare_W(run,2,h) + u_i;
       end
       
       % Community Friendly - Private
       tau = beta(h)*(Bh(h)+1)/N; mu = beta(h)/N; lambda = (N + nr_neighbors')*beta(h)/(N^2); %main parameters
       rho = 1./(2.*(tau+e));       
       ...
       price(h,:) = beta(h) * (Bh(h)*consumption_C(run,1,:,h)+total_consumption_C(1,run,h) + omega)/N;
       price_container(run,10,h,:) = price(h,:);

       cost(h) = kappa * total_consumption_C(1,run,h) ...
           * total_consumption_C(1,run,h)/N;
       revenue(h) = sum(price(h,:).* reshape(consumption_C(run,1,:,h),[1,N]),'all');
       realized_profit_rate(run,10,h) = revenue(h)/ cost(h);
       cost_container_C(run,1,h,1) = cost(h);

       revenue_container_C(run,1,h) = revenue(h);
       for i = 1:N
           u_i = -consumption_C(run,1,i,h)*price(h,i)+g(k,i)*consumption_C(run,1,i,h)...
               -e(i)*consumption_C(run,1,i,h)*consumption_C(run,1,i,h);
       ...
       price(h,:) = beta(h) * (Bh(h)*consumption_C(run,2,:,h)+total_consumption_C(2,run,h) + omega)/N;
       price_container(run,11,h,:) = price(h,:);

       cost(h) = kappa * total_consumption_C(2,run,h) ...
           * total_consumption_C(2,run,h)/N;
       revenue(h) = sum(price(h,:).* reshape(consumption_C(run,2,:,h),[1,N]),'all');
       realized_profit_rate(run,11,h) = revenue(h)/ cost(h);
       cost_container_C(run,2,h,1) = cost(h);

       revenue_container_C(run,2,h) = revenue(h);
       for i = 1:N
           u_i = -consumption_C(run,2,i,h)*price(h,i)+g(k,i)*consumption_C(run,2,i,h)...
               -e(i)*consumption_C(run,2,i,h)*consumption_C(run,2,i,h);
    ...
    for hyb = 1:10
        ...
        for h = 1:Hours
            ...
            for cs = 1:N
                if customers(cs) == 1
                    taup(cs) = beta(h)*(Bh(h)+1)/N; lambdap(cs) = beta(h)/N; mup(cs) = beta(h)/N;
                    taub(cs) = beta(h)*(Bh(h)+1)/N; lambdab(cs) = beta(h)/N; mub(cs) = beta(h)/N;
                elseif customers(cs) == 2
                    taup(cs) = beta(h)*(Bh(h)+1)/N; lambdap(cs) = 2*beta(h)/N; mup(cs) = beta(h)/N;
                    taub(cs) = beta(h)*(Bh(h)+1)/N; lambdab(cs) = 2*beta(h)/N; mub(cs) = beta(h)/N;
                ...
                elseif customers(cs) == 4
                    taup(cs) = beta(h)*(Bh(h)+1)/N; mup(cs) = beta(h)/N;
                    lambdap(cs) = (N + nr_neighbors(cs))*beta(h)/(N^2);
                    taub(cs) = beta(h)*(Bh(h)+1)/N; mub(cs) = beta(h)/N;
            ...
            if hyb == 1
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H1(run,1,:,h)+total_consumption_H1(1,run,h) + omega)/N;
               price_container(run,13,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H1(1,run,h) ...
                   * total_consumption_H1(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H1(run,1,:,h),[1,N]),'all');
               realized_profit_rate(run,13,h) = revenue(h)/ cost(h);
               cost_container_H1(run,1,h,1) = cost(h);
               revenue_container_H1(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H1(run,1,i,h)*price(h,i)+g(k,i)*consumption_H1(run,1,i,h)...
                       -e(i)*consumption_H1(run,1,i,h)*consumption_H1(run,1,i,h);
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H1(run,2,:,h)+total_consumption_H1(2,run,h) + omega)/N;
               price_container(run,14,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H1(2,run,h) ...
                   * total_consumption_H1(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H1(run,2,:,h),[1,N]),'all');
               realized_profit_rate(run,14,h) = revenue(h)/ cost(h);
               cost_container_H1(run,2,h,1) = cost(h);
               revenue_container_H1(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H1(run,2,i,h)*price(h,i)+g(k,i)*consumption_H1(run,2,i,h)...
                       -e(i)*consumption_H1(run,2,i,h)*consumption_H1(run,2,i,h);
            ...
            elseif hyb == 2
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H2(run,1,:,h)+total_consumption_H2(1,run,h) + omega)/N;
               price_container(run,16,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H2(1,run,h) ...
                   * total_consumption_H2(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H2(run,1,:,h),[1,N]),'all');
               realized_profit_rate(run,16,h) = revenue(h)/ cost(h);
               cost_container_H2(run,1,h,1) = cost(h);
               revenue_container_H2(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H2(run,1,i,h)*price(h,i)+g(k,i)*consumption_H2(run,1,i,h)...
                       -e(i)*consumption_H2(run,1,i,h)*consumption_H2(run,1,i,h);
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H2(run,2,:,h)+total_consumption_H2(2,run,h) + omega)/N;
               price_container(run,17,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H2(2,run,h) ...
                   * total_consumption_H2(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H2(run,2,:,h),[1,N]),'all');
               realized_profit_rate(run,17,h) = revenue(h)/ cost(h);
               cost_container_H2(run,2,h,1) = cost(h);
               revenue_container_H2(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H2(run,2,i,h)*price(h,i)+g(k,i)*consumption_H2(run,2,i,h)...
                       -e(i)*consumption_H2(run,2,i,h)*consumption_H2(run,2,i,h);
            ...
            elseif hyb == 3
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H3(run,1,:,h)+total_consumption_H3(1,run,h) + omega)/N;
               price_container(run,19,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H3(1,run,h) ...
                   * total_consumption_H3(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H3(run,1,:,h),[1,N]),'all');
               realized_profit_rate(run,19,h) = revenue(h)/ cost(h);
               cost_container_H3(run,1,h,1) = cost(h);
               revenue_container_H3(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H3(run,1,i,h)*price(h,i)+g(k,i)*consumption_H3(run,1,i,h)...
                       -e(i)*consumption_H3(run,1,i,h)*consumption_H3(run,1,i,h);
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H3(run,2,:,h)+total_consumption_H3(2,run,h) + omega)/N;
               price_container(run,20,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H3(2,run,h) ...
                   * total_consumption_H3(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H3(run,2,:,h),[1,N]),'all');
               realized_profit_rate(run,20,h) = revenue(h)/ cost(h);
               cost_container_H3(run,2,h,1) = cost(h);
               revenue_container_H3(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H3(run,2,i,h)*price(h,i)+g(k,i)*consumption_H3(run,2,i,h)...
                       -e(i)*consumption_H3(run,2,i,h)*consumption_H3(run,2,i,h);
            ...
            elseif hyb == 4
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H4(run,1,:,h)+total_consumption_H4(1,run,h) + omega)/N;
               price_container(run,22,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H4(1,run,h) ...
                   * total_consumption_H4(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H4(run,1,:,h),[1,N]),'all');
               realized_profit_rate(run,22,h) = revenue(h)/ cost(h);
               cost_container_H4(run,1,h,1) = cost(h);
               revenue_container_H4(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H4(run,1,i,h)*price(h,i)+g(k,i)*consumption_H4(run,1,i,h)...
                       -e(i)*consumption_H4(run,1,i,h)*consumption_H4(run,1,i,h);
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H4(run,2,:,h)+total_consumption_H4(2,run,h) + omega)/N;
               price_container(run,23,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H4(2,run,h) ...
                   * total_consumption_H4(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H4(run,2,:,h),[1,N]),'all');
               realized_profit_rate(run,23,h) = revenue(h)/ cost(h);
               cost_container_H4(run,2,h,1) = cost(h);
               revenue_container_H4(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H4(run,2,i,h)*price(h,i)+g(k,i)*consumption_H4(run,2,i,h)...
                       -e(i)*consumption_H4(run,2,i,h)*consumption_H4(run,2,i,h);
            ...
            elseif hyb == 5
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H5(run,1,:,h)+total_consumption_H5(1,run,h) + omega)/N;
               price_container(run,25,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H5(1,run,h) ...
                   * total_consumption_H5(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H5(run,1,:,h),[1,N]),'all');
               realized_profit_rate(run,25,h) = revenue(h)/ cost(h);
               cost_container_H5(run,1,h,1) = cost(h);
               revenue_container_H5(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H5(run,1,i,h)*price(h,i)+g(k,i)*consumption_H5(run,1,i,h)...
                       -e(i)*consumption_H5(run,1,i,h)*consumption_H5(run,1,i,h);
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H5(run,2,:,h)+total_consumption_H5(2,run,h) + omega)/N;
               price_container(run,26,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H5(2,run,h) ...
                   * total_consumption_H5(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H5(run,2,:,h),[1,N]),'all');
               realized_profit_rate(run,26,h) = revenue(h)/ cost(h);
               cost_container_H5(run,2,h,1) = cost(h);
               revenue_container_H5(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H5(run,2,i,h)*price(h,i)+g(k,i)*consumption_H5(run,2,i,h)...
                       -e(i)*consumption_H5(run,2,i,h)*consumption_H5(run,2,i,h);
            ...
            elseif hyb == 6
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H6(run,1,:,h)+total_consumption_H6(1,run,h) + omega)/N;
               price_container(run,28,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H6(1,run,h) ...
                   * total_consumption_H6(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H6(run,1,:,h),[1,N]),'all');
               realized_profit_rate(run,28,h) = revenue(h)/ cost(h);
               cost_container_H6(run,1,h,1) = cost(h);
               revenue_container_H6(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H6(run,1,i,h)*price(h,i)+g(k,i)*consumption_H6(run,1,i,h)...
                       -e(i)*consumption_H6(run,1,i,h)*consumption_H6(run,1,i,h);
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H6(run,2,:,h)+total_consumption_H6(2,run,h) + omega)/N;
               price_container(run,29,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H6(2,run,h) ...
                   * total_consumption_H6(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H6(run,2,:,h),[1,N]),'all');
               realized_profit_rate(run,29,h) = revenue(h)/ cost(h);
               cost_container_H6(run,2,h,1) = cost(h);
               revenue_container_H6(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H6(run,2,i,h)*price(h,i)+g(k,i)*consumption_H6(run,2,i,h)...
                       -e(i)*consumption_H6(run,2,i,h)*consumption_H6(run,2,i,h);
            ...
            elseif hyb == 7
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H7(run,1,:,h)+total_consumption_H7(1,run,h) + omega)/N;
               price_container(run,31,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H7(1,run,h) ...
                   * total_consumption_H7(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H7(run,1,:,h),[1,N]),'all');
               realized_profit_rate(run,31,h) = revenue(h)/ cost(h);
               cost_container_H7(run,1,h,1) = cost(h);
               revenue_container_H7(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H7(run,1,i,h)*price(h,i)+g(k,i)*consumption_H7(run,1,i,h)...
                       -e(i)*consumption_H7(run,1,i,h)*consumption_H7(run,1,i,h);
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H7(run,2,:,h)+total_consumption_H7(2,run,h) + omega)/N;
               price_container(run,32,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H7(2,run,h) ...
                   * total_consumption_H7(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H7(run,2,:,h),[1,N]),'all');
               realized_profit_rate(run,32,h) = revenue(h)/ cost(h);
               cost_container_H7(run,2,h,1) = cost(h);
               revenue_container_H7(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H7(run,2,i,h)*price(h,i)+g(k,i)*consumption_H7(run,2,i,h)...
                       -e(i)*consumption_H7(run,2,i,h)*consumption_H7(run,2,i,h);
            ...
            elseif hyb == 8
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H8(run,1,:,h)+total_consumption_H8(1,run,h) + omega)/N;
               price_container(run,34,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H8(1,run,h) ...
                   * total_consumption_H8(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H8(run,1,:,h),[1,N]),'all');
               realized_profit_rate(run,34,h) = revenue(h)/ cost(h);
               cost_container_H8(run,1,h,1) = cost(h);
               revenue_container_H8(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H8(run,1,i,h)*price(h,i)+g(k,i)*consumption_H8(run,1,i,h)...
                       -e(i)*consumption_H8(run,1,i,h)*consumption_H8(run,1,i,h);
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H8(run,2,:,h)+total_consumption_H8(2,run,h) + omega)/N;
               price_container(run,35,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H8(2,run,h) ...
                   * total_consumption_H8(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H8(run,2,:,h),[1,N]),'all');
               realized_profit_rate(run,35,h) = revenue(h)/ cost(h);
               cost_container_H8(run,2,h,1) = cost(h);
               revenue_container_H8(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H8(run,2,i,h)*price(h,i)+g(k,i)*consumption_H8(run,2,i,h)...
                       -e(i)*consumption_H8(run,2,i,h)*consumption_H8(run,2,i,h);
            ...
            elseif hyb == 9
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H9(run,1,:,h)+total_consumption_H9(1,run,h) + omega)/N;
               price_container(run,37,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H9(1,run,h) ...
                   * total_consumption_H9(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H9(run,1,:,h),[1,N]),'all');
               realized_profit_rate(run,37,h) = revenue(h)/ cost(h);
               cost_container_H9(run,1,h,1) = cost(h);
               revenue_container_H9(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H9(run,1,i,h)*price(h,i)+g(k,i)*consumption_H9(run,1,i,h)...
                       -e(i)*consumption_H9(run,1,i,h)*consumption_H9(run,1,i,h);
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H9(run,2,:,h)+total_consumption_H9(2,run,h) + omega)/N;
               price_container(run,38,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H9(2,run,h) ...
                   * total_consumption_H9(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H9(run,2,:,h),[1,N]),'all');
               realized_profit_rate(run,38,h) = revenue(h)/ cost(h);
               cost_container_H9(run,2,h,1) = cost(h);
               revenue_container_H9(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H9(run,2,i,h)*price(h,i)+g(k,i)*consumption_H9(run,2,i,h)...
                       -e(i)*consumption_H9(run,2,i,h)*consumption_H9(run,2,i,h);
            ...
            elseif hyb == 10
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H10(run,1,:,h)+total_consumption_H10(1,run,h) + omega)/N;
               price_container(run,40,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H10(1,run,h) ...
                   * total_consumption_H10(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H10(run,1,:,h),[1,N]),'all');
               realized_profit_rate(run,40,h) = revenue(h)/ cost(h);
               cost_container_H10(run,1,h,1) = cost(h);
               revenue_container_H10(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H10(run,1,i,h)*price(h,i)+g(k,i)*consumption_H10(run,1,i,h)...
                       -e(i)*consumption_H10(run,1,i,h)*consumption_H10(run,1,i,h);
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_H10(run,2,:,h)+total_consumption_H10(2,run,h) + omega)/N;
               price_container(run,41,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_H10(2,run,h) ...
                   * total_consumption_H10(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_H10(run,2,:,h),[1,N]),'all');
               realized_profit_rate(run,41,h) = revenue(h)/ cost(h);
               cost_container_H10(run,2,h,1) = cost(h);
               revenue_container_H10(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_H10(run,2,i,h)*price(h,i)+g(k,i)*consumption_H10(run,2,i,h)...
                       -e(i)*consumption_H10(run,2,i,h)*consumption_H10(run,2,i,h);
...
save './Results_Graph/sim6_avcons_S' average_consumption_S
save './Results_Graph/sim6_avcons_U' average_consumption_U
save './Results_Graph/sim6_avcons_W' average_consumption_W
save './Results_Graph/sim6_avcons_C' average_consumption_C
save './Results_Graph/sim6_avcons_H1' average_consumption_H1
save './Results_Graph/sim6_avcons_H2' average_consumption_H2
save './Results_Graph/sim6_avcons_H3' average_consumption_H3
save './Results_Graph/sim6_avcons_H4' average_consumption_H4
save './Results_Graph/sim6_avcons_H5' average_consumption_H5
save './Results_Graph/sim6_avcons_H6' average_consumption_H6
save './Results_Graph/sim6_avcons_H7' average_consumption_H7
save './Results_Graph/sim6_avcons_H8' average_consumption_H8
save './Results_Graph/sim6_avcons_H9' average_consumption_H9
save './Results_Graph/sim6_avcons_H10' average_consumption_H10
% Utility Containers
save './Results_Graph/sim6_utility_S' utility_S
save './Results_Graph/sim6_utility_U' utility_U
save './Results_Graph/sim6_utility_W' utility_W
save './Results_Graph/sim6_utility_C' utility_C
save './Results_Graph/sim6_utility_H1' utility_H1
save './Results_Graph/sim6_utility_H2' utility_H2
save './Results_Graph/sim6_utility_H3' utility_H3
save './Results_Graph/sim6_utility_H4' utility_H4
save './Results_Graph/sim6_utility_H5' utility_H5
save './Results_Graph/sim6_utility_H6' utility_H6
save './Results_Graph/sim6_utility_H7' utility_H7
save './Results_Graph/sim6_utility_H8' utility_H8
save './Results_Graph/sim6_utility_H9' utility_H9
save './Results_Graph/sim6_utility_H10' utility_H10
% Price Container
save './Results_Graph/sim6_price_container' price_container
% Revenue Containers
save './Results_Graph/sim6_revenue_container_S' revenue_container_S
save './Results_Graph/sim6_revenue_container_U' revenue_container_U
save './Results_Graph/sim6_revenue_container_W' revenue_container_W
save './Results_Graph/sim6_revenue_container_C' revenue_container_C
save './Results_Graph/sim6_revenue_container_H1' revenue_container_H1
save './Results_Graph/sim6_revenue_container_H2' revenue_container_H2
save './Results_Graph/sim6_revenue_container_H3' revenue_container_H3
save './Results_Graph/sim6_revenue_container_H4' revenue_container_H4
save './Results_Graph/sim6_revenue_container_H5' revenue_container_H5
save './Results_Graph/sim6_revenue_container_H6' revenue_container_H6
save './Results_Graph/sim6_revenue_container_H7' revenue_container_H7
save './Results_Graph/sim6_revenue_container_H8' revenue_container_H8
save './Results_Graph/sim6_revenue_container_H9' revenue_container_H9
save './Results_Graph/sim6_revenue_container_H10' revenue_container_H10
% Cost Containers
save './Results_Graph/sim6_cost_container_S' cost_container_S
save './Results_Graph/sim6_cost_container_U' cost_container_U
save './Results_Graph/sim6_cost_container_W' cost_container_W
save './Results_Graph/sim6_cost_container_C' cost_container_C
save './Results_Graph/sim6_cost_container_H1' cost_container_H1
save './Results_Graph/sim6_cost_container_H2' cost_container_H2
save './Results_Graph/sim6_cost_container_H3' cost_container_H3
save './Results_Graph/sim6_cost_container_H4' cost_container_H4
save './Results_Graph/sim6_cost_container_H5' cost_container_H5
save './Results_Graph/sim6_cost_container_H6' cost_container_H6
save './Results_Graph/sim6_cost_container_H7' cost_container_H7
save './Results_Graph/sim6_cost_container_H8' cost_container_H8
save './Results_Graph/sim6_cost_container_H9' cost_container_H9
save './Results_Graph/sim6_cost_container_H10' cost_container_H10
...
for run = 1:run_time
    ...
    omega_bar = 0;
    omega_variance = 2;
    profit_rate = [1.2; 1.4; 1.6; 1.8]; %Fixed profit rate
    ...
    Bh = repelem(Bh,6);
    Bh = Bh * (omega+4) / 4;
    profit_rate = [0.66; 0.8; 0.73; 0.87]; %Fixed profit rate
    profit_rate = repelem(profit_rate,6);
    beta(:) = profit_rate .* kappa .* ones(Hours,1);
    ...
    for hyb = 7:10
        ...
        for h = 1:Hours
            ...
            if hyb == 7
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_24(run,1,:,h)+total_consumption_24(1,run,h) + omega)/N;
               price_container_24(run,1,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_24(1,run,h) ...
                   * total_consumption_24(1,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_24(run,1,:,h),[1,N]),'all');
               realized_profit_rate_24(run,1,h) = revenue(h)/ cost(h);
               cost_container_24(run,1,h,1) = cost(h);
               revenue_container_24(run,1,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_24(run,1,i,h)*price(h,i)+g(k,i)*consumption_24(run,1,i,h)...
                       -e(i)*consumption_24(run,1,i,h)*consumption_24(run,1,i,h);
                   utility_24(run,1,i,h) = u_i;
                   welfare_24(run,1,h) = welfare_24(run,1,h) + u_i;
               end

               consumption_24(run,2,:,h) = b_HP'.*(g_bar(k)-omega_bar.*mup)' + a_HP'.*(g(k,:) - g_bar(k))';
               total_consumption_24(2,run,h) = sum(consumption_24(run,2,:,h));
               price(h,:) = beta(h) * (Bh(h)*consumption_24(run,2,:,h)+total_consumption_24(2,run,h) + omega)/N;
               price_container_24(run,2,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_24(2,run,h) ...
                   * total_consumption_24(2,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_24(run,2,:,h),[1,N]),'all');
               realized_profit_rate_24(run,2,h) = revenue(h)/ cost(h);
               cost_container_24(run,2,h,1) = cost(h);
               revenue_container_24(run,2,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_24(run,2,i,h)*price(h,i)+g(k,i)*consumption_24(run,2,i,h)...
                       -e(i)*consumption_24(run,2,i,h)*consumption_24(run,2,i,h);
            ...
            elseif hyb == 8
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_24(run,3,:,h)+total_consumption_24(3,run,h) + omega)/N;
               price_container_24(run,3,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_24(3,run,h) ...
                   * total_consumption_24(3,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_24(run,3,:,h),[1,N]),'all');
               realized_profit_rate_24(run,3,h) = revenue(h)/ cost(h);
               cost_container_24(run,3,h,1) = cost(h);
               revenue_container_24(run,3,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_24(run,3,i,h)*price(h,i)+g(k,i)*consumption_24(run,3,i,h)...
                       -e(i)*consumption_24(run,3,i,h)*consumption_24(run,3,i,h);
                   utility_24(run,3,i,h) = u_i;
                   welfare_24(run,3,h) = welfare_24(run,3,h) + u_i;
               end

               consumption_24(run,4,:,h) = b_HP'.*(g_bar(k)-omega_bar.*mup)' + a_HP'.*(g(k,:) - g_bar(k))';
               total_consumption_24(4,run,h) = sum(consumption_24(run,4,:,h));
               price(h,:) = beta(h) * (Bh(h)*consumption_24(run,4,:,h)+total_consumption_24(4,run,h) + omega)/N;
               price_container_24(run,4,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_24(4,run,h) ...
                   * total_consumption_24(4,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_24(run,4,:,h),[1,N]),'all');
               realized_profit_rate_24(run,4,h) = revenue(h)/ cost(h);
               cost_container_24(run,4,h,1) = cost(h);
               revenue_container_24(run,4,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_24(run,4,i,h)*price(h,i)+g(k,i)*consumption_24(run,4,i,h)...
                       -e(i)*consumption_24(run,4,i,h)*consumption_24(run,4,i,h);
            ...
            elseif hyb == 9
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_24(run,5,:,h)+total_consumption_24(5,run,h) + omega)/N;
               price_container_24(run,5,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_24(5,run,h) ...
                   * total_consumption_24(5,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_24(run,5,:,h),[1,N]),'all');
               realized_profit_rate_24(run,5,h) = revenue(h)/ cost(h);
               cost_container_24(run,5,h,1) = cost(h);
               revenue_container_24(run,5,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_24(run,5,i,h)*price(h,i)+g(k,i)*consumption_24(run,5,i,h)...
                       -e(i)*consumption_24(run,5,i,h)*consumption_24(run,5,i,h);
                   utility_24(run,5,i,h) = u_i;
                   welfare_24(run,5,h) = welfare_24(run,5,h) + u_i;
               end

               consumption_24(run,6,:,h) = b_HP'.*(g_bar(k)-omega_bar.*mup)' + a_HP'.*(g(k,:) - g_bar(k))';
               total_consumption_24(6,run,h) = sum(consumption_24(run,6,:,h));
               price(h,:) = beta(h) * (Bh(h)*consumption_24(run,6,:,h)+total_consumption_24(6,run,h) + omega)/N;
               price_container_24(run,6,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_24(6,run,h) ...
                   * total_consumption_24(6,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_24(run,6,:,h),[1,N]),'all');
               realized_profit_rate_24(run,6,h) = revenue(h)/ cost(h);
               cost_container_24(run,6,h,1) = cost(h);
               revenue_container_24(run,6,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_24(run,6,i,h)*price(h,i)+g(k,i)*consumption_24(run,6,i,h)...
                       -e(i)*consumption_24(run,6,i,h)*consumption_24(run,6,i,h);
            ...
            elseif hyb == 10
               ...
               price(h,:) = beta(h) * (Bh(h)*consumption_24(run,7,:,h)+total_consumption_24(7,run,h) + omega)/N;
               price_container_24(run,7,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_24(7,run,h) ...
                   * total_consumption_24(7,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_24(run,7,:,h),[1,N]),'all');
               realized_profit_rate_24(run,7,h) = revenue(h)/ cost(h);
               cost_container_24(run,7,h,1) = cost(h);
               revenue_container_24(run,7,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_24(run,7,i,h)*price(h,i)+g(k,i)*consumption_24(run,7,i,h)...
                       -e(i)*consumption_24(run,7,i,h)*consumption_24(run,7,i,h);
                   utility_24(run,7,i,h) = u_i;
                   welfare_24(run,7,h) = welfare_24(run,7,h) + u_i;
               end

               consumption_24(run,8,:,h) = b_HP'.*(g_bar(k)-omega_bar.*mup)' + a_HP'.*(g(k,:) - g_bar(k))';
               total_consumption_24(8,run,h) = sum(consumption_24(run,8,:,h));
               price(h,:) = beta(h) * (Bh(h)*consumption_24(run,8,:,h)+total_consumption_24(8,run,h) + omega)/N;
               price_container_24(run,8,h,:) = price(h,:);
               cost(h) = kappa * total_consumption_24(8,run,h) ...
                   * total_consumption_24(8,run,h)/N;
               revenue(h) = sum(price(h,:).* reshape(consumption_24(run,8,:,h),[1,N]),'all');
               realized_profit_rate_24(run,8,h) = revenue(h)/ cost(h);
               cost_container_24(run,8,h,1) = cost(h);
               revenue_container_24(run,8,h) = revenue(h);
               for i = 1:N
                   u_i = -consumption_24(run,8,i,h)*price(h,i)+g(k,i)*consumption_24(run,8,i,h)...
                       -e(i)*consumption_24(run,8,i,h)*consumption_24(run,8,i,h);
...
save './Results_Graph/sim6_avcons_24' average_consumption_24
% Utility Containers
save './Results_Graph/sim6_utility_24' utility_24
% Price Container
save './Results_Graph/sim6_price_container_24' price_container_24
% Revenue Containers
save './Results_Graph/sim6_revenue_container_24' revenue_container_24
% Cost Containers
save './Results_Graph/sim6_cost_container_24' cost_container_24
